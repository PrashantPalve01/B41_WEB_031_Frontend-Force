...
async function fetchProducts() {
  try {
    const response = await fetch('https://product-data-371b9-default-rtdb.firebaseio.com/classic.json');
...
function renderProducts(products) {
  ...
  products.forEach(product => {
    ...
    productCard.onclick = () => {
      window.location.href = `product_details.html?id=${product.id}&category=classic`;
    };
